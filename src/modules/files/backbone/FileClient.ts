...
export class FileClient extends RESTClientAuthenticate {
    ...
    public async getFiles(
        request: BackboneGetFilesRequest
    ): Promise<ClientResult<Paginator<BackboneGetFilesResponse>>> {
    ...
    public async downloadFile(id: string): Promise<ClientResult<any>> {
        return await this.download<any>(`/api/v1/Files/${id}`)
    }
}
