...
export class PublicAPITest extends AbstractTest {
    public run(): void {
        ...
        publicFunctions[AccountController.name] = [
            ...
            nameof<AccountController>((r) => r.createIdentityAndDevice),
        ...
        publicFunctions[CertificateIssuer.name] = [
            nameof<CertificateIssuer>((r) => r.init),
            nameof<CertificateIssuer>((r) => r.issueCertificate)
        ]
        ...
        publicFunctions[SyncController.name] = [
            nameof<SyncController>((r) => r.init),
            nameof<SyncController>((r) => r.sync),
            nameof<SyncController>((r) => r.getLastCompletedSyncTime)
        ]
        ...
        publicFunctions[FileController.name] = [
            ...
            nameof<FileController>((r) => r.loadPeerFileByTruncated),
            nameof<FileController>((r) => r.loadPeerFileByReference),
            nameof<FileController>((r) => r.loadPeerFile),
        ...
        publicFunctions[RelationshipsController.name] = [
            ...
            nameof<RelationshipsController>((r) => r.requestTermination),
        ...
        function testPublicFunctions(controllerName: string) {
            it(`${controllerName} should expose the correct API`, function () {
                let found = 0
                ...
                expect(found).to.equal(publicFunctions[controllerName].length)
            })
        }
